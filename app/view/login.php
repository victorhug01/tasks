...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Verifica se o formulário foi submetido
    if (empty($_POST['userEmail']) || empty($_POST['userPassword'])) {
        $_SESSION['not_authenticated'] = "Erro: Email ou senha não preenchidos."; // Armazenar o erro na sessão
        header('Location: ../../index.php?p=login'); // Redireciona para o login
        exit();
    } else {
        // Se ambos os campos estão preenchidos, processar o login
        ...
        if($row == 1){
            ...
            $_SESSION['not_authenticated'] = "Erro: Usuário ou senha inválidos."; // Armazenar o erro na sessão
...
<body>
    ...
    <section class="d-flex w-100 align-items-center justify-content-center" style="height: calc(100vh - 100px);">
        <form action="" method="POST" class="container d-grid gap-2 col-xl-4 col-lg-4 vh-75">
            ...
            <?php
                unset($_SESSION['not_authenticated']); // Limpa a variável de sessão após exibição
            endif;
            ?>
